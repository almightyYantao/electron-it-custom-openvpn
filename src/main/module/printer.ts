...
ipcMain.on(EVENT_PRINTER_INSTALL, (_event: IpcMainEvent, arg: string) => {
  xiaokuDebug('开始安装打印机')
  ...
  if (process.platform === 'darwin') {
    installByMac(ip, name)
      .then((result) => {
        _event.sender.send(EVENT_PRINTER_INSTALL_SUCCESS)
      })
      .catch((reason) => {
        _event.sender.send(EVENT_PRINTER_INSTALL_ERROR, reason)
      })
...
function installByWindows(ip: string, name: string): Promise<boolean> {
  return new Promise((resolve, rejects) => {
    global.mainWindow.webContents.send(EVENT_PRINTER_DOWNLOAD_INFO, '开始注册打印机端口信息')
    try {
      ...
      sudo.exec(
        ...
        function (error, stdout, stderr) {
          ...
          global.mainWindow.webContents.send(EVENT_PRINTER_DOWNLOAD_INFO, '重启打印机服务')
          sudo.exec(
            ...
            function (error, stdout, stderr) {
              ...
              global.mainWindow.webContents.send(EVENT_PRINTER_DOWNLOAD_INFO, '开始安装打印机')
              ...
              resolve(true)
              global.mainWindow.webContents.send(EVENT_PRINTER_INSTALL_SUCCESS)
            }
          )
        }
      )
      ...
      global.mainWindow.webContents.send(EVENT_PRINTER_INSTALL_ERROR, e)
      resolve(false)
      rejects(e)
    }
  })
